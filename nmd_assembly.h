...
const uint8_t op1modrm[] = { 0xFF, 0x62, 0x63, 0x69, 0x6B, 0xC0, 0xC1, 0xC6, 0xC7, 0xD0, 0xD1, 0xD2, 0xD3, 0xF6, 0xF7, 0xFE };
...
bool nmd_x86_decode_buffer(const void* const buffer, const size_t bufferSize, NMD_X86Instruction* const instruction, const NMD_X86_MODE mode, const uint32_t featureFlags)
{
	...
	if (*b == 0x0F) /* 2 or 3 byte opcode. */
	{
		...
		if (*b == 0x38 || *b == 0x3A) /* 3 byte opcode. */
		{
			...
			instruction->opcodeMap = *b == 0x38 ? NMD_X86_OPCODE_MAP_0F_38 : NMD_X86_OPCODE_MAP_0F_3A;
	...
	else /* 1 byte opcode */
	{
		...
		const NMD_Modrm modrm = instruction->modrm;
			...
			if (featureFlags & NMD_X86_FEATURE_FLAGS_VALIDITY_CHECK)
			{
				...
				{
					if (mode == NMD_X86_MODE_64 && instruction->flags.fields.hasModrm && modrm.fields.mod != 0b11)
						return false;
				}
				...
				if (featureFlags & NMD_X86_FEATURE_FLAGS_OPERANDS)
				{
					...
					if (instruction->numOperands > 0)
					{
						...
						else if (op >= 0x91 && op <= 0x97)
						{
							...
							instruction->operands[1].type = NMD_X86_OPERAND_TYPE_REGISTER;
							instruction->operands[1].fields.reg = instruction->flags.fields.operandSize64 ? NMD_X86_REG_RAX : (instruction->prefixes & NMD_X86_PREFIXES_OPERAND_SIZE_OVERRIDE && mode != NMD_X86_MODE_16 ? NMD_X86_REG_AX : NMD_X86_REG_EAX);
							instruction->operands[0].action = instruction->operands[1].action = NMD_X86_OPERAND_ACTION_READ_WRITE;
						}
...
void nmd_x86_format_instruction(const NMD_X86Instruction* const instruction, char buffer[], const uint64_t runtimeAddress, const uint32_t formatFlags)
{
		...
		if (instruction->encoding == NMD_X86_INSTRUCTION_ENCODING_EVEX)
		{
...
#endif
			{
				...
				else if (op == 0xff) /* Opcode extensions Group 5 */
				{
					...
					*si.buffer++ = ' ';
					if (instruction->modrm.fields.mod == 0b11)
						appendString(&si, (si.instruction->flags.fields.operandSize64 ? reg64 : (operandSize ? reg16 : reg32))[si.instruction->modrm.fields.rm]);
					else
	...
	else if (instruction->opcodeMap == NMD_X86_OPCODE_MAP_0F_38)
	{
		...
		{
			appendString(&si, instruction->simdPrefix == NMD_X86_PREFIXES_OPERAND_SIZE_OVERRIDE ? "adcx" : (instruction->simdPrefix == NMD_X86_PREFIXES_REPEAT ? "adox" : (instruction->flags.fields.operandSize64 ? "wrssq" : "wrssd")));
			*si.buffer++ = ' ';
			if (!instruction->simdPrefix)
			{
				appendEy(&si);
